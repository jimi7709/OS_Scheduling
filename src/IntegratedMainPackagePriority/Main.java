package IntegratedMainPackagePriority;
...
class SchedulingAlgorithm {
    public static List<Result> run(List<Process> jobList, List<Result> resultList) {
        ...
        while (completedProcesses < n) {
            ...
            for (Process process : jobList) {
                if (process != currentProcess && process.arriveTime <= runTime && process.remainingTime > 0) {
                    process.priority = Math.max(0, process.originalPriority - (runTime - process.arriveTime));
                }
            }
