...
class Priority {
	public static void run(List<Process> jobList) {
		...
		Process pi;// 초기값(리스트의 특정요소)
		int piIn;// 초기값(리스트의 특정요소의 인덱스)
		int k = 0; // 반복문 제어변수
		// while (temp.size() != 0) {// 모든 프로세스가 처리될때까지 반복
		for (int i = 0; i < temp.size(); i++) {// 우선순위가 가장 큰 거 찾기
			pi = temp.get(i);
			piIn = i;
			if (i <= temp.size() - 1) {// IndexOutofBounds 조건을 만족하면
				if (pi.arriveTime <= runTime) {// 해당 프로세스가 도착했고 
					if (pi.priority < temp.get(i).priority) {// 우선순위가 더 크다면
						pi = temp.get(i);
						piIn = i;
					}
