package IntegratedMainPackageSJF;
//비 선점형 SJF
        ...
        while (jobList.size() != 0 || readyQueue.size() != 0 || currentProcess != 0) {
            ...
            if (currentProcess == 0) {
                if (readyQueue.size() != 0) {
                    // SJF: 가장 짧은 Burst Time을 가진 프로세스를 선택
                    ReadyQueueElement shortestJob = readyQueue.get(0);
                    for (ReadyQueueElement element : readyQueue) {
                        if (element.burstTime < shortestJob.burstTime) {
                            shortestJob = element;
                        }
                    }

                    Result result = new Result(shortestJob.processID, runTime + shortestJob.burstTime, shortestJob.burstTime, shortestJob.waitingTime);

                    resultList.add(result);

                    currentProcess = shortestJob.processID;
                    cpuTime = 0;
                    readyQueue.remove(shortestJob);
                }
            } else {
                cpuTime++;
                if (cpuTime == resultList.get(resultList.size() - 1).getCompletionTime() - runTime) {
                    currentProcess = 0;
                }
            }
